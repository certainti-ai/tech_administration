...
| R13 | Tenant DDL drift &mdash; tables | every `trd365_<digits>` schema should carry the same table set | **union is 143 tables; only 81 real tables exist in all 30.** Four schemas (`_00411`, `_00428`, `_00440`, `_00486`) carry **no case machinery at all** &mdash; all 19 case tables absent. Six lack `account_interactions` / `ai_assessment_error` / `interaction_type`; six lack `project_qre_adjustment_history` |
| R14 | Tenant DDL drift &mdash; columns | a table present in two schemas should have the same columns | **15 tables differ.** `attachments`: 12 of 30 schemas carry `country_rid`/`state_rid`, 18 do not. `cases`: one schema alone has `illinois_cost_of_supplies` / `illinois_lease_costs_of_computers`. `ai_technical_summary`: one schema alone has `generated_on` |
| R15 | Ad-hoc tables in tenant schemas | production tenant schemas should hold only generated tables | **36 backup/staging tables across 28 schemas** &mdash; `bkup20260805_*`, `chs_bkp_histfix_20260731`, `project_fiscal_temp_2f1b4937`, `history_staging_*`, `r082506_*`, `temp_4de37235`. Several exist in a single schema |
