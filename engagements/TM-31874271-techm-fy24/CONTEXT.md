...
- **CORRECTED Aug 26 — the #REF! finding is withdrawn.** It came from `Financial details for June 7 2026 response.xlsx`, which was **not** the submission. The workbook actually sent is `Tech Mahindra FY24 Audit Financials.xlsx` (Appendix 1 items 3 and 7): 48 MB, six sheets — Salary, Timesheet, Subcon Cost, Pivot - Subcon data, SUBCON DATA, Project POCs — and **zero error cells anywhere**. Never repeat the #REF! claim.
- **What was actually submitted, and the real defect.** The workbook carried a clean 12-month PeopleSoft timesheet extract (67,344 rows, all 366 days). But it was **scoped to the 184 employees on the Salary sheet, who are all P1/P2 people**, rather than to the seven claimed project codes. Resulting coverage against what existed: **P1 74,134 hrs / 40 emps = 94%**; **P3 21,702 / 11 = 24%** (incidental — those people also charged to P3); **P2, P4, P5, P6, P7 = zero**. Salary sheet covers P1 and P2 only (237 people, $7,746,349). Subcon Cost covers P1 and P2 only (28 individuals, $1,417,216 gross, $138,179 claimed) — so **P3–P6's $166,574 of contract cost has never had a single document behind it**.
- **The four decks, examined.** Only `Wireless-Audit_CRA-Updated.pptx` predates the audit — created 22 Dec 2022, which also predates FY2024 (Apr 2023). `Project 1 Wireless - Evidence.pptx` was created and saved 27 seconds apart on the submission date and opens by admitting old emails could not be retrieved; its QE timeline dates (11 and 13 Apr 2024) fall in FY2025. `Tech Mahindra Canada FY24 Audit Response.pptx` was **generated by PptxGenJS** (a script), 21 slides, 2,176 words, **zero images**, last saved by "Cole Rutherford". `Screenshot Proof-R&D` (CRA's Reference 5) was created 29 May 2026 by "Ashok Ramesh1 - EXT", after the review meetings.
- **THE SUBMISSION CONTRADICTS ITSELF — most serious finding in the file.** Audit Response deck slide 5 tells CRA in writing: "Time-based allocation: Employee timesheets track SR&ED vs. non-SR&ED hours per project", with a table showing **100%** against Principal Consultant, Project Lead and SME on P1. The workbook sent the same day applies a **flat 15%** to those same people with no such tracking. Slide 12 names four P1 claimants: Ramesh Makolu (on the salary sheet but as *Tech Manager*, at 15% not 100%), **Ashok Ramesh — not on the salary sheet**, **Vigenshwar Lakshminarayana — not on the salary sheet**, Vineet Banga (subcon sheet, 15%). Establish why two named claimants are absent from the salary schedule before CRA asks.
- **"Available" evidence that was never produced.** Deck slides 13 and 19 mark against the T661 evidence codes — project planning, design/architecture/**source code**, progress reports, and for P2 signed contracts, SOWs, automation scripts, LeanFT/Karate implementation guides, sprint logs, retrospectives, defect tracking — all marked *Available*. None reached CRA. If any of it exists, producing it is the strongest remaining move on P1.
- **The recoverable fact:** 34,132 hours across 23 employees for P4–P7 existed in the client's own system on the day the response was filed. Re-running the same extract filtered by project code instead of by employee closes the hours half of the May 7 request. That is the highest-value hour of work outstanding.
- The submitted Subcon Cost sheet confirms the 0.65 factor (cost x 0.15 x 0.65 = claimed) and carries an additional unexplained `R&D %` column of 0.2 on every row.
