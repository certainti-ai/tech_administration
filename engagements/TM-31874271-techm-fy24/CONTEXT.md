...
- **UNRESOLVED — which financial workbook CRA received.** `Financial details for June 7 2026 response.xlsx` in the client's audit folder carries 3,660 `#REF!` cells at `D14:O320` on sheet `2- Emp hours by Project` (twelve monthly hour columns x 305 employees; each cell sums thirteen deleted references). The Aug 24 revision has the identical broken block. **CRA's Appendix 1 lists a file of that exact name as received on June 4 (item 4)**, alongside `Tech Mahindra FY24 Audit Financials.xlsx` (items 3 and 7). **The client states that file was not shared** and that the financial support given to CRA was `Tech Mahindra FY24 Audit Financials.xlsx` — **which we do not hold.** Do not assert the #REF! point in any submission until this is resolved: obtain the Audit Financials workbook as sent, reconcile against Appendix 1 item 4, and if needed ask the Financial Reviewer which files are on the Agency's record.
